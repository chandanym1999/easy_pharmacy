...
.ll {
	...
	<div class="main-area"> 
		...
		<div class="login">
			...
			<form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.8;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" >
            <h2>Admin Panel - Lab Order Details</h2>
                <br>
                ...
                <tbody>
                    ...
                    $con=mysqli_connect("localhost","root","");
						...
						mysqli_select_db($con,'easy_pharmacy');
						 $resul = mysqli_query($con,"SELECT distinct cusname FROM laborder;") or die("Failed to fetch".mysql_error());
                    while($row1= mysqli_fetch_assoc( $resul)){
						$x=$row1['cusname'];
						$result = mysqli_query($con,"SELECT * FROM laborder where cusname='$x';") or die("Failed to fetch".mysql_error());
							while( $row = mysqli_fetch_assoc( $result) ){
				
                                    echo "<tr><td>{$row['orderId']}</td><td>{$row['labname']}</td><td>{$row['cusname']}</td><td>{$row['cusmob']}</td><td>{$row['cusadd']}</td><td><a href=\"delete3.php?id=".$row['orderId']."\">Delete</a></td></tr>\n";
                                        }
