    ...
    <div class="main-area">
        ...
        <div class="login">
			...
			<form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.9;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" >
                <h2>Admin Panel - Add Labs</h2>
			  <div class="form-group center">
				<label for="user" style="font-weight: bold;color: #000000;">Lab Name:</label>
				<input type="text" class="form-control" id="user" name="labname" style="width:50%;margin-left: 24%" required>
			  </div>
			  <div class="form-group center">
				<label for="email" style="font-weight: bold;color: #000000;">Lab Price:</label>
				<input type="text" class="form-control" id="email" name="labprice" style="width:50%;margin-left: 24%" required>
			  </div>

                
			  
			  <button type="submit" class="btn btn-default">Add/Update Lab</button>
                <br>

                <br>
                <a href="adpharlist.php">See List of List</a>
			</form> 
			<?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST"){
				...
				$result = mysqli_query($conn,"SELECT * FROM labs where labName='".$labName."';");
                //$row = mysqli_fetch_array($result);

                if(mysqli_num_rows($result)==0)
				{               $sql1="insert into labs(labName,labprice) values('".$labName."', '".$labprice."');";
