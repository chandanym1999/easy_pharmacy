            ...
            <form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.8;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;">
                ...
                <thead>
                    <tr style="padding-left: 10px;">
                        <th style="border:1px solid black">Lab ID</th>
                        <th style="border:1px solid black">Lab Name</th>
                        <th style="border:1px solid black">Lab Price</th>
                ...
                <tbody>
                    ...
                    mysqli_select_db($con,'easy_pharmacy');
                        ...
                        $resul = mysqli_query($con,"SELECT * FROM labs;") or die("Failed to fetch".mysql_error());
                    
                    while( $row = mysqli_fetch_assoc( $resul) ){
                                    echo "<tr><td>{$row['labId']}</td><td>{$row['labName']}</td><td>{$row['labprice']}</td><td><a href=\"delete1.php?id=".$row['labId']."\">Delete</a></td></tr>\n";
                                        }
                    ?>
                  </tbody>
                </table>
