...
.ll {
    ...
    <?php
                        ...
                        $result = mysqli_query($con,"SELECT * FROM customerlogin where cuUserName='".$_SESSION["uname"]."';") or die("Failed to Login".mysql_error());

                        $row = mysqli_fetch_array($result);

                        $cuname=$row["cuUserName"];
                        $cupass=$row["cuPassword"];
						$cusid=$row["cuId"];
    ?>
	...
	<div class="main-area"> 
		...
		<div class="login"> 
			...
			<form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.9;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" >
			  ...
			  <div class="form-group center">
				<label for="user" style="font-weight: bold;color: #000000;">Customer User Name:</label>
				<input type="text" class="form-control" id="user" name="cusname" value="<?php echo"$cuname" ?>" style="width:50%;margin-left: 24%" placeholder="<?php echo"$cuname" ?>" required>
			  </div>
			  <div class="form-group center">
				<label for="email" style="font-weight: bold;color: #000000;">Customer Password:</label>
				<input type="text" class="form-control" id="email" name="cuspass" style="width:50%;margin-left: 24%" placeholder="<?php echo"$cupass" ?>" required>
			  </div>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST"){
                ...
                $sql1="UPDATE customerlogin SET cuUserName = '".$cusName."', cuPassword= '".$cuspass."' WHERE CuId = '".$cusid."';";
